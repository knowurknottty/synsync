...
const ComparisonChart: React.FC = () => {
  ...
  const getLevelColor = (level: string) => {
    const colors: Record<string, string> = { 'II': 'border-blue-500 text-blue-400 bg-blue-500/10', 'III': 'border-purple-500 text-purple-400 bg-purple-500/10', 'III-IV': 'border-pink-500 text-pink-400 bg-pink-500/10' };
  ...
  return (
    <div className="w-full bg-neuro-900/50 border border-neuro-700/50 rounded-2xl p-6 md:p-8 shadow-xl backdrop-blur-sm">
      <div className="text-center mb-10 border-b border-neuro-700/50 pb-8">
        <h2 className="text-3xl md:text-4xl font-bold mb-3 text-white">SynSync Protocol Library</h2>
        <p className="text-gray-400">Explore the specifications of all 21 SynSync sessions</p>
      </div>
      ...
      <div className="flex flex-wrap gap-4 mb-8 bg-neuro-800/30 p-4 rounded-xl border border-neuro-700/30">
        ...
        <div className="flex flex-col gap-1 min-w-[140px] flex-1">
          ...
          <select value={filterLevel} onChange={(e) => setFilterLevel(e.target.value)} className="bg-neuro-900 border border-neuro-700 text-gray-200 text-sm rounded-lg p-2.5 focus:ring-neuro-500 focus:border-neuro-500 block w-full">
            <option value="all">All Levels</option><option value="II">Level II (Foundational)</option><option value="III">Level III (Advanced)</option><option value="III-IV">Level III-IV (Deep)</option>
          </select>
        </div>
        <div className="flex flex-col gap-1 min-w-[140px] flex-1">
          ...
          <select value={filterEvidence} onChange={(e) => setFilterEvidence(e.target.value)} className="bg-neuro-900 border border-neuro-700 text-gray-200 text-sm rounded-lg p-2.5 focus:ring-neuro-500 focus:border-neuro-500 block w-full">
            <option value="all">All Types</option><option value="EVIDENCE">Clinical Evidence</option><option value="RESEARCH">Research Based</option>
          </select>
        </div>
      </div>
          ...
          <tbody>
            {filteredAndSorted.map((session, idx) => (
              <tr key={idx} className="border-b border-neuro-800/50 hover:bg-white/5 transition-colors even:bg-white/[0.02]">
                ...
                <td className="px-6 py-4"><span className={`text-[10px] uppercase font-bold tracking-wider ${session.evidence === 'EVIDENCE' ? 'text-green-400' : 'text-blue-400'}`}>{session.evidence}</span></td>
