...
export const Visualizer: React.FC<VisualizerProps> = ({ 
    ...
    const initCanvas2D = (currentMode: string) => {
        ...
        // PEAK CALCULATOR
        const calculatePeak = (data: Uint8Array) => {
            let max = 0;
            for(let i=0; i<data.length; i++) {
        ...
        const render = () => {
            ...
            if (currentMode === 'spectrum') {
                ...
                const drawScope = (data: Uint8Array, color: string, label: string, xOff: number, yOff: number, w: number, h: number, freq: number, rmsDb: number, peak: number) => {
                    ctx.save();
                    ctx.translate(xOff, yOff);
                    
                    // DIGITAL TELEMETRY FOOTER
                    const footerH = 25;
                    const scopeH = h - footerH;
                    
                    // Footer BG
                    ctx.fillStyle = '#050507'; // Dark Void
                    ctx.fillRect(0, scopeH, w, footerH);
                    ctx.strokeStyle = '#222';
                    ctx.lineWidth = 1;
                    ctx.strokeRect(0, scopeH, w, footerH);
                    ...
                    const freqStr = freq > 0 ? `${freq.toFixed(1)}Hz` : '----';
                    const peakStr = `PK:${peak.toFixed(2)}`;
                    
                    ctx.fillText(freqStr, 5, h - 8);
                    ctx.textAlign = 'center';
                    ctx.fillText(dbStr, w/2, h - 8);
                    ctx.textAlign = 'right';
                    ctx.fillText(peakStr, w - 5, h - 8);
                    
                    // Live Indicator
                    if (rmsDb > -50) {
                        ctx.fillStyle = color;
                        ctx.fillRect(w - 6, scopeH + 6, 2, 2);
                    }
                    ...
                    // Border
                    ctx.strokeStyle = '#333'; ctx.strokeRect(0,0,w,scopeH);
                    ctx.restore();
                };
                ...
                // Stereo Width Meter
                const stereoWidth = Math.min(100, Math.abs(rmsL - rmsR) * 10 + 50); // Rough approximation
                ctx.fillStyle = '#333'; ctx.fillRect(rect.width - 110, 15, 100, 10);
                ctx.fillStyle = '#00E5FF'; ctx.fillRect(rect.width - 110, 15, stereoWidth, 10);
                ctx.font = '9px monospace'; ctx.fillStyle = '#AAA'; ctx.fillText('STEREO FIELD', rect.width - 110, 12);
                
                ctx.restore();

                // SUB PANELS
                const ySub = mainH;
                drawScope(timeL, '#FFB000', 'CH1: LEFT', 0, ySub, subW, subH, freqL, rmsL, peakL);
                drawScope(timeR, '#FF3D00', 'CH2: RIGHT', subW, ySub, subW, subH, freqR, rmsR, peakR);
                drawScope(timeAux, '#00E5FF', 'CH3: AUX', subW*2, ySub, subW, subH, freqAux, rmsAux, peakAux);
            }
